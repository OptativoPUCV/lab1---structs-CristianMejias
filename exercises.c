...
{
  *newSize = 0;
  int *nuevoArr = malloc(sizeof(int));
  ...
  for (int i = 0 ; i < size ; i++)
  {
    if (arr[i] % 2 == 0)
    {
      ...
      *newSize += 1;
      ...
      nuevoArr = (int *) realloc(nuevoArr, sizeof(int) * *newSize);
    }
  }
  return nuevoArr;
}
                       ...
                       int result[])
{
  ...
  while (k < size1 + size2)
  {
    ...
    if (i == size1) result[k] =	arr2[j];
    if (j == size2) result[k] =	arr2[i];
    k++;
  }
}
...
int checkSorted(int arr[], int size)
{
  ...
  {
      if (arr[i] > arr[i + 1]) esAscendente = false;
      if (arr[i] < arr[i + 1]) esDescendente = false;
  }
